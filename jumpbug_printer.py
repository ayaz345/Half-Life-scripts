...
def jb(fps, waits):
    i = 2
    script = (
        f'alias jb1 "fps_max {str(fps)}'
        + ';w;+jump;w;-jump;alias @jb jb2;bxt_append _jb"\n'
    )
    for _ in range(0, waits):
        script += (
            f'alias jb{str(i)} "w;alias @jb jb{str(i + 1)}'
            + ';bxt_append _jb"\n'
        )
        i += 1
    ...
    script += (
        f'alias jb{str(i)}'
        + ' "+duck;w 5;say jumpbug!;-duck;+jump;w;-jump;alias @jb"\n\n'
    )
